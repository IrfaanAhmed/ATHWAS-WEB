...
const SettingController = {
  ...
  getSettings: async (req, res) => {
    try {
      ...
      SettingServices.getRecordsList(findPattern)
        .then((settingdata) => {
          if (settingdata) {
            let resMsg = "Success";
  ...
  updateSetting: async (req, res) => {
    try {
      ...
      var normal_start_delivery_time =
        req.body.normal_start_delivery_time || "";
      ...
      if (errors.length > 0) {
        ...
        SettingServices.oneRecord(findPattern)
          .then(async (settingdata) => {
            if (settingdata) {
              var updatePattern = {
                ...
                vat: vat,
                premium_order_amount: normal_order_amount,
                premium_start_delivery_time: normal_start_delivery_time,
                premium_end_delivery_time: normal_end_delivery_time,
              ...
              SettingServices.updateRecord(findPattern, updatePattern)
                .then((updatedRes) => {
                  // success
                  let resMsg = Messages.SETTING_UPDATE_SUCCESS;
  ...
  getDeliverySettings: async (req, res) => {
    try {
      ...
      DeliveryServices.getRecordsList(findPattern)
        .then((settingdata) => {
          if (settingdata) {
            let resMsg = "Success";
      ...
      await settingsData.reduce(async (promise, setting, key) => {
        ...
        var createDeliverySettings = {
          ...
          max_special_order_delivery_time:
            setting.max_special_order_delivery_time,
        };

        await DeliveryServices.createRecord(createDeliverySettings)
          .then(async (result) => {})
          .catch((err) => {
            let resMsg = Validation.getErrorMessage(err);
            Response.send(req, res, 500, resMsg);
