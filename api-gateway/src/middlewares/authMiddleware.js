...
const authCheck = {
      ...
      if (config.validateApiAccess) {
        if (req.method != "OPTION") {
          ...
          if (typeof accessToken !== "undefined") {
            const requestTime = Encryption.cryptLibDecryption(
              accessToken,
              config.apiAccessKey
            );
  ...
  checkJWTAuthentication: async (req, res, next) => {
    try {
      ...
      var internal_api_access_key =
        req.headers.internal_api_access_key || false;
      ...
      if (
        config.validateApiAccess &&
        !nonLoginTypes.includes(userType) &&
        !nonLoginModules.includes(moduleName) &&
        !nonLoginApis.includes(submoduleName) &&
        !nonLoginApis.includes(api_name)
      ) {
        if (authToken && authToken != "") {
          try {
            var decodedToken = jwt.decode(authToken, config.jwtSecretKey);
